...
public class HandymanController {
    ...
    public ResponseEntity<MessageResponse> switchToHandyman(@RequestBody BecomeHandymanRequest handymanRequest){
        HandymanDTO handymanDTO =
                Utils.copyProperties(userService.getUserByUsername(jwtFilter.getUsername()),new HandymanDTO());
        ...
        if(handymanService.switchToHandyman(handymanDTO))
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new MessageResponse("contact data and job title are obligatory"));
    }
